...
export function buildDiscoveryPage(key, opts) {
  ...
  const duplicatesFamilyGuide = key === 'met-kinderen';
  const indexable = !duplicatesFamilyGuide && results.length >= DISCOVERY_MIN_INDEXABLE_RESULTS;
  return { config, results, indexable, robots: indexable ? undefined : 'noindex,follow', updatedAt: ymdInAmsterdam(opts?.now || new Date()), canonical: duplicatesFamilyGuide ? '/museumgids/kindvriendelijke-musea-amsterdam' : config?.path };
}
