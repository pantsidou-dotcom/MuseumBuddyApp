...
async function fetchMuseumsByIds(ids) {
  ...
  let { data, error } = await supabaseClient.from('musea').select(MUSEUM_SELECT_COLUMNS).in('id', ids);
...
async function loadExhibitions() {
  ...
  try {
    const buildQuery = (selectColumns) =>
      supabaseClient.from('exposities').select(selectColumns).order('start_datum', { ascending: true });

    let { data, error } = await buildQuery(EXHIBITION_WITH_MUSEUM_SELECT);

    if (error && error.message && /column|identifier|relationship/i.test(error.message)) {
      ({ data, error } = await buildQuery(EXHIBITION_FALLBACK_SELECT));
    }
    ...
    const missingSlugs = [
      ...new Set(missingSlugRows.map((row) => deriveSlug(row)).filter(Boolean)),
    ];
    ...
    return { exhibitions: [], error: 'unknown' };
  }
}

function useExhibitions() {
  const [state, setState] = useState({ loading: true, exhibitions: [], error: null });

  useEffect(() => {
    let active = true;

    async function run() {
      const result = await loadExhibitions();
      ...
      if (result.error) {
        setState({ loading: false, exhibitions: [], error: result.error });
      } else {
        setState({ loading: false, exhibitions: result.exhibitions, error: null });
      }
    }
      ...
      {error ? (
        ...
        <ul className="grid" style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {Array.from({ length: 6 }).map((_, index) => (
            <li key={`exhibition-skeleton-${index}`}>
              <SkeletonMuseumCard />
            </li>
          ))}
        </ul>
