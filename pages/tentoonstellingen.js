...
export default function ExhibitionsPage({ exhibitions = [], error = null }) {
  ...
  useEffect(() => {
    ...
    loadExhibitions(supabaseClient)
      .then(({ exhibitions: freshExhibitions, error: freshError }) => {
        if (cancelled) return;
        setExhibitionState({
          exhibitions: freshExhibitions,
          error: freshError,
          loading: false,
        });
      })
  ...
  }, [exhibitionState.error, exhibitionState.exhibitions.length, exhibitionState.loading]);
...
export async function getStaticProps() {
  const { exhibitions: loadedExhibitions, error } = await loadExhibitions(supabaseClient);
...
async function loadExhibitions(client) {
  if (!client) {
    return {
      exhibitions: [],
